...
def main():
    ...
    if model_testing:
        ...
        agent_trainer = AgentTrainer(agent_type="ppo_agent", iteration="attempt_0", study_name="study_1_trial_11", plotting=plotting, rendering=rendering)
        game = Game(rendering=rendering)  # Create a game for the env
        env = agent_trainer.create_envs(game, delta_time=(0.01666), frame_repeats=4)[0]  # first env, just one agent
        agent = agent_trainer.load_agent(env.envs[0])
        ...
        # Set save_freq and print_freq to a very high value to effectively avoid saving.
        reward_logger = RewardLoggerCallback(
            print_freq=10000,  # Unrealistically high to prevent frequent printing
            save_freq=100_000_000,  # Same high value to prevent saving
            patience=float('inf'),  # Infinite patience ensures no early stopping
            agent=agent,
            model_file=agent_trainer.model_file,
            steps_file=agent_trainer.steps_file,
            avg_reward_file=agent_trainer.avg_reward_file,
